...
AI 代理是**系统**，通过赋予**大型语言模型（LLMs）**访问**工具**和**知识**来扩展其能力，从而使其能够**执行操作**。
...
> 说明：这里的“环境-传感器-执行器”描述的是代理系统在运行时的结构；图中“知识-访问工具-执行行动”描述的是 LLM 在该系统中被增强后的能力。两者是不同层次的拆分方式，互为补充而不是矛盾关系。
